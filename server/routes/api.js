...
router.post('/new', function(req, res) {
	...
	expense
		.save()
		.then((err, expense) => res.send(`Spent ${expense.amount} on ${expense.name}`) )
})
...
router.put('/update/group', function(req, res) {
	Expense.findOneAndUpdate(
		...
		{ new: true }
	).exec((err, expense) => res.send(`Changed group of ${expense.name} to ${expense.group}`) )
})
...
router.put('/update/amount', function(req, res) {
	Expense.findOneAndUpdate(
		...
		{ new: true }
	).exec((err, expense) => res.send(`Changed amount of ${expense.name} to ${expense.amount}`)	)
})

router.delete('/delete', function(req, res) {
	Expense.findOneAndDelete({ name: req.body.name })
	.exec((err, expense) => res.send(`Removed ${expense.name} from expenses`))
})
